    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<ArticleWithState>>(
        stream: widget.articleRepository.watchArticles(
          ArticleQuery(scope: _scope),
        ),
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            return const Center(child: Text('Could not load the timeline.'));
          }
          if (!snapshot.hasData) {
...
class _ArticleTile extends StatelessWidget {
  const _ArticleTile({required this.item, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      trailing: item.state.isStarred
          ? const Icon(Icons.star, semanticLabel: 'Saved')
          : null,
    );
  }
}
